...
- Never run `npx quartz upgrade`, `npx quartz sync` or `npx quartz create` here. They operate on the whole git repo, not on `site/`. Upgrade with `npm run site:upgrade`.
